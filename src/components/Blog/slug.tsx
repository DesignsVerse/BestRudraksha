...
const BlogDetailslug = ({ blog }: { blog: any }) => {
  const [activeSection, setActiveSection] = useState<string>("");

  if (!blog) {
    return <p className="text-center text-red-500">No blog found</p>;
  }
  ...
  const renderContent = (section: any, index: number) => (
    <div key={index} id={`section-${index}`} className="mb-10 scroll-mt-20">
      ...
      {index === 2 && blog.productId && <Advertisement />}
    </div>
  );
    ...
    <Provider store={store}>
      <>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData(blog)) }}
        />
            ...
            <div className="flex flex-col lg:flex-row gap-7.5 xl:gap-12.5">
              <div className="lg:max-w-[750px] w-full">
                ...
                <div>
                  ...
                  {blog.sections.map((section: any, index: number) => renderContent(section, index))}
                  <div className="rounded-xl bg-white pt-7.5 pb-6 px-4 sm:px-7.5 my-10 shadow-sm">
                    ...
                    <a href="#" className="flex items-center justify-center gap-3 mt-5.5">
                      <div className="flex w-12 h-12 rounded-full overflow-hidden">
                        <Image src={blog.author.avatar} alt={blog.author.name} width={48} height={48} />
                      </div>
                      <div>
