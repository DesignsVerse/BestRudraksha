...
const BlogItem = ({ blog }: { blog: BlogItem }) => {
  return (
    <div className="shadow-1 bg-white rounded-xl px-4 sm:px-5 pt-5 pb-4 h-full flex flex-col">
      <Link 
        href={`/blog/${blog.slug}`} 
        className="rounded-md overflow-hidden block aspect-[4/3]"
      >
        <Image
          src={blog.img}
          alt="blog"
          ...
          width={330}
          height={248} // 4:3 aspect ratio (330x248)
          priority={false}
        />
      </Link>

      <div className="mt-5.5 flex-grow flex flex-col">
        ...
        <h2 className="font-medium text-dark text-lg sm:text-xl ease-out duration-200 mb-4 hover:text-blue line-clamp-2">
