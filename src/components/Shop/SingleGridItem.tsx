...
const SingleGridItem = ({ item }: { item: Product }) => {
  ...
  const [isWishlisted, setIsWishlisted] = useState(false); // Move state inside component
  ...
  const handleItemToWishList = () => {
    setIsWishlisted(!isWishlisted); // Toggle the state first
    dispatch(
      addItemToWishlist({
        ...item,
        status: "available",
  ...
  id="favOne"
  className={`flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 ${isWishlisted ? 'bg-[#800000] text-white' : 'bg-white text-dark'}`}
>
  <svg
    className="fill-current"
    width="16"
