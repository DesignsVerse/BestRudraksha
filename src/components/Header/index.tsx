...
const Header: React.FC = () => {
  ...
  // Icons (unchanged, included for completeness)
  const SearchIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className || "text-black hover:text-[#800000] transition-colors duration-200"}>
  ...
  const HeartIcon: React.FC = () => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-black hover:text-[#800000] transition-colors duration-200">
      <path d="M20.84 4.61C20.3291 4.09904 19.7226 3.69347 19.0492 3.41325C18.3759 3.13303 17.6492 2.98312 16.91 2.973C15.14 2.973 13.54 3.893 12.73 5.293C11.92 3.893 10.32 2.973 8.55 2.973C7.81076 2.98312 7.08408 3.13303 6.41075 3.41325C5.73742 3.69347 5.13092 4.09904 4.62 4.61C1.86 7.37 2.27 11.91 6.55 15.99L12.73 22.17L18.91 15.99C23.19 11.91 23.6 7.37 20.84 4.61Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
  ...
  const CloseIcon: React.FC = () => (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-[#800000]">
  ...
  return (
    <header className={`fixed left-0 top-0 w-full z-9999 bg-[#FFFAF5] transition-all ease-in-out duration-300 ${stickyMenu && "shadow-lg border-b border-[#800000]/20"}`}>
      <div className="max-w-[1160px] mx-auto px-4 sm:px-7.5 xl:px-0 relative">
        <div className={`flex items-center justify-between ease-out duration-200 ${stickyMenu ? "py-4" : "py-6"}`}>
          {/* Mobile Menu Toggle */}
          <div className="xl:hidden flex items-center">
            ...
            >
              {navigationOpen ? <CloseIcon /> : (
                <svg className="fill-current text-[#800000]" width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="flex-shrink-0">
            <Link href="/">
              <div className="relative h-15 w-50 filter brightness-75 contrast-125">
                <Image
                ...
                {menuData.map((menuItem: Menu) =>
                  menuItem.submenu ? (
                    ...
                    >
                      <Link
                        href={menuItem.path || "#"}
                        className={`text-[#800000] hover:text-[#800000]/80 text-custom-sm font-medium flex ${stickyMenu ? "xl:py-4" : "xl:py-6"}`}
                      >
                        {menuItem.title}
                      </Link>
                    </li>
                ...
                {searchQuery && (
                  <div className="max-h-[70vh] overflow-y-auto px-4 pb-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {searchResults.length > 0 ? (
                      ...
                      <div className="col-span-full py-4 text-center text-[#800000]/70">No products found</div>
                    )}
                  </div>
                )}
              </div>
